...
Adafruit_NeoPixel led_strips[] = {
  ...
  Adafruit_NeoPixel(num_neopixels, neopixel_pins[2]),  
  Adafruit_NeoPixel(num_neopixels, neopixel_pins[3])
};
...
void light_strip(int red, int green, int blue){
  int color_number = get_color_number(red, green, blue);
  int light_row = get_light_row(color_number);
  int start_led = get_start_led(color_number);
  int end_led = get_end_led(color_number);

  for (int i = start_led; i < end_led; i++) {
    led_strips[light_row].setPixelColor(i, led_strips[light_row].Color(red, green, blue));
  }
  led_strips[light_row].show();
}

int get_color_number(int red, int green, int blue) {
  for ( int i = 0; i < num_colors; i++ ) {
    RGB color = all_colors[i];
    if ( color.r == red &&
      color.g == green &&
      color.b == blue ) {
      return i;
...
// LOWELL
int get_start_led(int col_num) {
  int something_madeup = ( num_neopixels % col_num ) * 2;
  return something_madeup;
}

// LOWELL
int get_end_led(int col_num) {
  int something_madeup = ( num_neopixels % col_num ) * 6;
  return something_madeup;
}


// void old_light_strip(int red, int green, int blue) {
//   int colrnum;
//   int pixsize = 15;
//   int pixstart;
//   int pixend;
